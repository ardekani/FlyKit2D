...
void CBlobResult::PrintBlobs( char *nom_fitxer ) const
{
    double_stl_vector area, /*perimetre,*/ exterior, compacitat, longitud, 
        externPerimeter, perimetreConvex, perimetre;//, allid;
    int i;
    ...
    for(i=0; i<GetNumBlobs(); i++)
    {
        fprintf( fitxer_sortida, "blob %d ->\t a=%7.0f\t p=%8.2f (%8.2f extern)\t pconvex=%8.2f\t ext=%.0f\t m=%7.2f\t c=%3.2f\t l=%8.2f\n",
            i, area[i], perimetre[i], externPerimeter[i], perimetreConvex[i], exterior[i], compacitat[i], longitud[i]);//,allid[i] );
    }
...
void CBlobResult::splitMergedBlobs(CBlobResult &dst,double thresh2, double thresh3) 
{
    ...
    for (int bc = 0;bc<numBlobs;bc++)
    {
        ...
        currentBlob = GetBlob(bc);
        //std::vector<CBlob*> separatedBlobs;
        //std::vector<CvSeq*> separatedContours;
        int n = currentBlob->numberOfMergedBlobs(thresh2,thresh3);
        if(n<2)
        ...
        else
        {
            ...
            {
                cvZero(tmpForContour);
                for (int k = 0; k<clusteredPoints[cnt].size();k++)
                    cvCircle(tmpForContour,cvPoint((int)clusteredPoints[cnt][k].x,(int)clusteredPoints[cnt][k].y),2 ,cvScalar(255),3);

                cvShowImage("tmpForContour",tmpForContour);
                CBlobResult tmpBlobRes;
                tmpBlobRes = CBlobResult(tmpForContour,NULL, 0);
                if (tmpBlobRes.m_blobs.size()==0)
                    printf("\nNo Blob is found! ");
                else
                {
                    subCurrentBlob = tmpBlobRes.GetBlob(0);
                    dst.m_blobs.push_back(new CBlob( subCurrentBlob));
                }

            }
            cvReleaseImage(&tmpForContour);
...
//
void CBlobResult::removeNthBlob(int Nth)
{
        Blob_vector::iterator itBlobs = m_blobs.begin();
        for( int i = 0; i < Nth; i++ )
            itBlobs++;
        delete *itBlobs;
        m_blobs.erase(m_blobs.begin() + Nth,m_blobs.begin()+Nth + 1);
}
void CBlobResult::setBlobsOrigImage(const IplImage* maskedFrame) 

{
    ...
    for (int bc = 0;bc<numBlobs;bc++)
    {
        //		printf("\nbc = %d",bc);
        currentBlob = GetBlob(bc);
        CvRect blbrct = currentBlob->GetBoundingBox();//GetBoundsRect();
        //		printf("blbrct (h=%d,w=%d,x=%d,y=%d)",blbrct.height,blbrct.width,blbrct.x,blbrct.y);
        if((blbrct.height ==0) || (blbrct.width==0))
            return;

        //		printf("\nafter get bounding");
        IplImage* tempImg = cvCloneImage(maskedFrame);
        cvSetImageROI(tempImg,blbrct);
        //		printf("\nafter set ROI");
        CvSize sz = cvGetSize(tempImg);
        //		printf("\nafter getting Size");
        if (currentBlob->blobOrigImage)
            cvReleaseImage(&currentBlob->blobOrigImage);

        //		printf("\nsz.w =%d , sz. h = %d, (%d, %d)",sz.width,sz.height,tempImg->depth,tempImg->nChannels);	
        currentBlob->blobOrigImage = cvCreateImage(cvGetSize(tempImg),tempImg->depth,tempImg->nChannels);
        //		printf("\ncurrblb w =%d , h = %d, d = %d, ch = %d",currentBlob->blobOrigImage->width,currentBlob->blobOrigImage->height,currentBlob->blobOrigImage->depth, currentBlob->blobOrigImage->nChannels);
        cvZero(currentBlob->blobOrigImage);
        //		printf("\n tempImg w = %d, h = %d, blobOrigImage w = %d, h = %d\n",sz.width,sz.height, currentBlob->blobOrigImage->width,currentBlob->blobOrigImage->height);
        cvCopy(tempImg, currentBlob->blobOrigImage, NULL);
        //		printf("\n after cvCopy");
...
{
    int numBlobs = this->GetNumBlobs();
    CBlob *currentBlob, subCurrentBlob;
    currentBlob = GetBlob(Nth);
    std::vector<std::vector<CvPoint2D32f>> clusteredPoints;
    ...
    for (size_t cnt = 0;cnt<clusteredPoints.size();cnt++)//separatedContours.size();cnt++)
    {
        cvZero(tmpForContour);
        for (int k = 0; k<clusteredPoints[cnt].size();k++)
            cvCircle(tmpForContour,cvPoint((int)clusteredPoints[cnt][k].x,(int)clusteredPoints[cnt][k].y) ,0,cvScalar(255));
    ...
    std::cout<< "\n  this->GetNumBlobs() " <<this->GetNumBlobs();
    getchar();
    cvReleaseImage(&tmpForContour);
